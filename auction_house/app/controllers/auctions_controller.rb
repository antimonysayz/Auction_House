class AuctionsController < ApplicationController
    ...
    post '/auctions' do
        ...
        if params[:name].empty? || params[:description].empty? || params[:current_bid].empty?
            ...
            @auction = Auction.create(:name => params[:name], :description => params[:description], :current_bid => params[:current_bid], :image_url => params[:image_url], :created_by => @user.username, :user_id => @user.id, :end_date => params[:end_date])
            redirect '/bids'
    end
